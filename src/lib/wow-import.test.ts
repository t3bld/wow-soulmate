...
test("database import lifecycle attaches privately, rejects stale runs, expires and deletes without resurrection", { skip: process.env.WOW_IMPORT_DB_TEST !== "1" }, async () => {
  ...
  try {
    ...
    assert.deepEqual(await getProfile(subject), { ...multiple, about: "", notificationEmail: null });
    ...
    await saveProfile(subject, { ...multiple, discoverable: true, notificationEmail: " player@example.com " });
    assert.equal((await getProfile(subject))?.notificationEmail, "player@example.com");
    assert.equal(Object.hasOwn(await database().profile.findUniqueOrThrow({ where: { subject } }), "notificationEmail"), false);
    ...
    await saveProfile(subject, { ...multiple, notificationEmail: "updated@example.com" });
    assert.equal((await getProfile(subject))?.notificationEmail, "updated@example.com");
    await saveProfile(subject, { ...multiple, notificationEmail: "" });
    assert.equal((await getProfile(subject))?.notificationEmail, null);
